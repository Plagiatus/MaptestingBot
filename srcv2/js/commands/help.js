    ...
    execute: function test(message, args) {
        ...
        if (!args.length) {
            ...
            for (let c of d.commands.values()) {
                if (fitsCriteria(c, searchCriteria))
                    useableCommands.push(c);
            }
        ...
        // command specific help command
            ...
            let command = util_1.findCommandWithAlias(args[0].toLowerCase());
            if (!command) {
